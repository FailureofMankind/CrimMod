...
{
    public class true_soul_render : ModProjectile  
    { 
		...
		public override void OnHitNPC(NPC target, int damage, float knockBack, bool crit)
		{
		    ...
		    target.AddBuff(39, 800); //inflicts "cursed inferno" debuff
			Main.PlaySound(SoundID.NPCHit36, projectile.position);        
            if(Main.rand.Next(9)==0)
            {
            Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, Main.rand.Next(-7, 7), Main.rand.Next(-7, 7), mod.ProjectileType("true_soul_render_proj"), (int)(projectile.damage / 5), 0, Main.myPlayer); //Spawning a projectile
            }
        ...
        public override void AI()
        {
            ...
            {
            Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, Main.rand.Next(-7, 7), Main.rand.Next(-7, 7), mod.ProjectileType("true_soul_render_proj"), (int)(projectile.damage / 3), 0, Main.myPlayer); //Spawning a projectile
            }

            int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 75);  //this is the dust that this projectile will spawn
